<template>
  <!-- no-route-dismiss: questo dialog si apre anche da un parametro della
       query (?modifica=anagrafica), e subito dopo la pagina riscrive la
  ...
  <q-dialog v-model="aperto" no-route-dismiss @before-show="carica">
    <q-card style="min-width: 520px; max-width: 640px">
      <q-card-section>
        ...
        <div class="vp-hint">
          Nascita e residenza servono ai documenti generati (atto di
          subentro, cessione di fabbricato). L'anagrafica è del profilo:
          vale per tutti gli immobili in cui compare.
        </div>
      </q-card-section>
        ...
        <div class="row q-col-gutter-sm q-mt-none">
          <div class="col-4">
            <q-input
              ...
              dense
              clearable
              data-testid="owner-data-nascita"
            />
          </div>
          <div class="col-4">
            <q-input
        ...
        <div class="row q-col-gutter-sm q-mt-none">
          ...
          <div class="col-6">
            <q-input
              ...
              outlined
              dense
              data-testid="owner-codice-fiscale"
            />
          </div>
        </div>
      ...
      <q-card-actions align="right">
        ...
        <q-btn
          ...
          label="Salva"
          :loading="salvataggio"
          data-testid="salva-anagrafica-owner"
          @click="salva"
        />
      </q-card-actions>
    </q-card>
...
const $q = useQuasar();
const form = ref<FormAnagraficaOwner>(vuoto());
...
async function carica() {
  ...
  errore.value = '';
  try {
    const { data } = await api.get<Partial<FormAnagraficaOwner>>(
      `/api/v1/owners/${props.ownerId}/`,
    );
    const dati = vuoto();
    for (const chiave of Object.keys(dati) as Array<keyof FormAnagraficaOwner>) {
...
async function salva() {
  ...
  const f = form.value;
  try {
    await api.patch(`/api/v1/owners/${props.ownerId}/`, {
      ...f,
